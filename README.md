...
- A strong AI opponent: transposition table, null-move pruning, late move reductions,
  principal variation search, quiescence with delta pruning and a tapered evaluation.
  It runs in a background worker, so the board stays smooth while it thinks.
- Four material styles taken from the original bundle: Wood, Marble, Metal, Grass.
- Drag a piece or tap-then-tap; pieces lift off the board under your finger and settle
  when they land. Undo, hint, flip, PGN copy, captured-piece tray, material score.
...
node tactics.js    # tactical suite, new engine vs the previous one
node selfplay.js   # engine vs engine, plus make/unmake integrity
node book.js       # opening book sanity check
```

`perft.js` passes 30/30, including Kiwipete to depth 4 (4,085,603 nodes) and the start
position to depth 5 (4,865,609 nodes). Perft is the standard way to prove a move generator
is exactly correct, and it still passes after the incremental Zobrist hashing was added.

## Strength

The search was rewritten around a transposition table with incremental Zobrist hashing,
plus null-move pruning, late move reductions, principal variation search, check extensions,
killer moves and a history heuristic. The evaluation is tapered between middlegame and
endgame and scores passed, doubled and isolated pawns, the bishop pair, rooks on open files
and the king's pawn shield.

The effect, measured on the same tactical suite: the previous engine at its **hardest**
setting solved 6 of 7 in 24.2 seconds. The current engine at its **easiest** setting solves
the same 6 of 7 in 0.9 seconds. At equal time it reaches roughly twice the depth — depth 13
where the old one reached depth 6.

So the ladder now starts where the old one topped out:

| Level | Max depth | Time budget |
|---|---|---|
| Strong | 9 | 1.5 s |
| Expert | 13 | 3 s |
| Master | 17 | 6 s |
...
No level plays deliberately badly — there are no random blunders and no crippled search.
Levels differ only in how long and how deep they think.
...
the bundle, so the engine here is an independent implementation. See Strength above.

**A note on the worker.** A page loaded from `file://` has an opaque origin, so Chromium refuses
to construct a `Worker` from a script URL — which is why the engine module is written as one
named function. The UI stringifies it, wraps it in a Blob and starts the worker from that, which
Chromium does allow. If worker creation fails for any reason the search falls back to the main
thread, so the app still works.
